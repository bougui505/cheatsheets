...
\vspace{1ex}
\noindent\rule{.33\textwidth}{0.4pt}

\tikzset{
  strikeoutnode/.style={
    draw,
    path picture={
      \draw[black,line width=0.5pt,solid]
        (path picture bounding box.north west) --
...
\begin{tikzpicture}
  \node[strikeoutnode]{strikeoutnode}; 
\end{tikzpicture}
\begin{minipage}[c]{3cm}
\begin{verbatim}
\tikzset{
...
\subsection{Grids}
From: \url{https://tex.stackexchange.com/a/39698/19419}
  ...
  grid with coordinates/.style={
    to path={%
      \pgfextra{%
        ...
        \pgfmathsetmacro{\grd@xb}{\the\pgf@x/1cm}
        \pgfmathsetmacro{\grd@yb}{\the\pgf@y/1cm}
        \pgfmathsetmacro{\grd@xc}{\grd@xa \
            + \pgfkeysvalueof{/tikz/grid with coordinates/major step}}
        \pgfmathsetmacro{\grd@yc}{\grd@ya \
            + \pgfkeysvalueof{/tikz/grid with coordinates/major step}}
        \foreach \x in {\grd@xa,\grd@xc,...,\grd@xb}
        \node[anchor=north] at (\x,\grd@ya) {\pgfmathprintnumber{\x}};
        \foreach \y in {\grd@ya,\grd@yc,...,\grd@yb}
